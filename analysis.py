...
for (stint, compound), stint_data in clean_laps.groupby(["Stint", "Compound"]):
    ax.scatter(stint_data["TyreLife"], stint_data["LapTimeSeconds"], label=f"stint{int(stint)} - {compound}")
...
for index, row in plot_results.iterrows():
    ...
    ax.scatter(row["TyreEffect"], index, s=marker_size, zorder=zorder)
    
ax.axvline(validated_median, linestyle="--", linewidth=1.5, label=f"Field median ({validated_median:.3f} s/lap)")
...
fig, ax = plt.subplots(figsize=(9, 5))
ax.plot(strategy_results["PitLap"], strategy_results["DeltaToBest"], marker="o")
ax.axvline(best_pit_lap, linestyle="--", linewidth=1.5, label=f"Best pit lap: {best_pit_lap}")
...
fig, ax = plt.subplots(figsize=(9, 5))
ax.bar(pit_lap_probability.index, pit_lap_probability.values)
ax.axvline(best_pit_lap, linestyle="--", linewidth=1.5, label=f"Nominal optimum: lap {best_pit_lap}")
